"""
OpenDataLab 数据集同步到 MinIO

功能：将 OpenDataLab 数据集上传到内网 MinIO 对象存储

工作方式：
逐个文件处理：下载单个文件 → 上传到MinIO → 立即删除临时文件
...
MINIO_TARGET_PREFIX = ''  # 留空则使用数据集名称，或指定自定义前缀
MINIO_SECURE = False
...
SKIP_EXISTING = True  # 是否跳过 MinIO 中已存在的文件
...
def sync_file(file_path, minio_client, bucket, object_name, use_memory=True):
    """
    同步单个文件
    ...
    if FILE_LIST_MODE == 'progressive':
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if use_memory and file_size < MEMORY_THRESHOLD:
                ...
                print(f"  -> 上传到 MinIO...")
                minio_client.put_object(bucket, object_name, file_data, length=file_size)
            else:
                print(f"  -> 上传到 MinIO...")
...
def main():
    ...
    try:
        ...
        print(f"✓ MinIO 连接成功: {MINIO_ENDPOINT}")
    ...
    # 确定目标前缀
    target_prefix = MINIO_TARGET_PREFIX if MINIO_TARGET_PREFIX else DATASET_REPO.split('/')[-1]
    print(f"  MinIO 存储路径: {MINIO_BUCKET}/{target_prefix}/\n")
    ...
    for idx, file_path in enumerate(files_to_sync, 1):
        ...
        try:
            ...
            bytes_transferred = sync_file(
                file_path, minio_client, MINIO_BUCKET, object_name, use_memory
            )
